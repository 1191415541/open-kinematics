...
from suspension_kinematics import cli
from suspension_kinematics.core.enums import PointID
from suspension_kinematics.gui.app import (
    KinematicsWorkbenchApp,
    _ReportCurveSelector,
    _ReportExportDialog,
)
from suspension_kinematics.gui.suspension import SuspensionWorkbenchPage
...
def test_suspension_page_exposes_load_solve_and_curve_controls() -> None:
    ...
    assert re.search(
        r"workspace\.add\(\s*workspace_left,\s*weight=self\.WORKSPACE_PREVIEW_WEIGHT",
        layout_source,
    )
    assert re.search(
        r"workspace\.add\(\s*workspace_right,\s*weight=self\.WORKSPACE_SIDE_WEIGHT",
        layout_source,
    )
    assert re.search(
        r'controls = ttk\.LabelFrame\(\s*workspace_left,\s*text="Simulation Input",\s*padding=8',
        layout_source,
    )
...
def test_suspension_hardpoint_table_is_compact_and_auto_sized() -> None:
    ...
    assert re.search(r"self\.main_panedwindow\.add\(\s*left,\s*weight=0", layout_source)
    assert re.search(
        r"self\.main_panedwindow\.add\(\s*right,\s*weight=5", layout_source
    )
...
def test_gui_project_save_dialogs_use_shared_project_extension() -> None:
    from suspension_kinematics.gui.steering import file_actions as steering_files
    from suspension_kinematics.gui.suspension import app as suspension_app
...
def test_suspension_page_has_optimization_actions() -> None:
    ...
    assert re.search(
        r"output_actions = ttk\.Frame\(\s*output_frame\s*\)",
        optimization_content_source,
    )
...
def test_cli_exposes_unified_gui_entrypoint() -> None:
    ...
    assert "suspension_kinematics.gui.app" in source
    assert "uv run --extra viz suspension-kinematics gui" in source

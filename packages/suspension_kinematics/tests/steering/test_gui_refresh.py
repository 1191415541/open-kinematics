...
from suspension_kinematics.gui.steering import app as steering_app
from suspension_kinematics.gui.steering import widgets as steering_widgets
...
class _FakeRoot:
    ...
    def after_cancel(self, handle) -> None:
        ...
        self.after_calls = [item for item in self.after_calls if item[2] != handle]
...
def test_pitman_transform_commit_applies_geometry_and_notifies_once() -> None:
    ...
    rows_by_name = {(row.category, row.name): row for row in controls.rows}

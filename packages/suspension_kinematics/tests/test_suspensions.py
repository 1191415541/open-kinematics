...
from suspension_kinematics.constraints import DistanceConstraint
from suspension_kinematics.core.enums import PointID, ShimType, Units
from suspension_kinematics.core.types import make_vec3
...
from suspension_kinematics.suspensions.double_wishbone_carrier import (
...
class TestDoubleWishboneCarrierSuspension:
    ...
    def test_constraints_keep_trackrod_and_axle_on_upright(
        ...
        for carrier_point in (
            ...
            assert (
                frozenset((PointID.TRACKROD_OUTBOARD, carrier_point))
                not in distance_pairs
            )
            assert (
                frozenset((PointID.AXLE_INBOARD, carrier_point)) not in distance_pairs
            )
            assert (
                frozenset((PointID.AXLE_OUTBOARD, carrier_point)) not in distance_pairs
            )
        carrier_pairs = {
            frozenset(
                (PointID.LOWER_WISHBONE_OUTBOARD, PointID.CARRIER_STEERING_AXIS_LOWER)
            ),
            frozenset(
                (PointID.UPPER_WISHBONE_OUTBOARD, PointID.CARRIER_STEERING_AXIS_UPPER)
            ),
            frozenset(
                (
                    PointID.CARRIER_STEERING_AXIS_LOWER,
            ...
            frozenset(
                (PointID.LOWER_WISHBONE_OUTBOARD, PointID.CARRIER_STEERING_AXIS_UPPER)
            ),
            frozenset(
                (PointID.UPPER_WISHBONE_OUTBOARD, PointID.CARRIER_STEERING_AXIS_LOWER)
            ),
        }
        assert carrier_pairs <= distance_pairs

    def test_instant_centers_return_points(
        self, valid_carrier_hardpoints, valid_config
    ):
        """

...
from suspension_kinematics.gui.common import (
    OptimizationCancelledError,
    parse_float_entry,
    raise_if_cancelled,
)
from suspension_kinematics.gui.project import (
    build_project_document,
    write_project_document,
)
from suspension_kinematics.steering.csv_loader import (
    load_two_segment_steering_hardpoints_rows,
)
...
from suspension_kinematics.steering.limits import (
...
from suspension_kinematics.steering.outputs import (
...
def _hardpoint_row_from_data(row: object) -> SteeringHardpointRow:
    """Deserialize one editable hardpoint row without losing default rows."""
    if isinstance(row, SteeringHardpointRow):
        return replace(row)
    if not isinstance(row, Mapping):
        raise TypeError("Steering hardpoint rows must be mappings")
    return SteeringHardpointRow(
...
def _curve_from_data(curve: object) -> SteeringCurve:
    """Deserialize one steering curve definition."""
    if isinstance(curve, SteeringCurve):
        return replace(curve)
    if not isinstance(curve, Mapping):
        raise TypeError("Steering curves must be mappings")
    return SteeringCurve(
        x_output=cast(str, curve["x_output"]),
        y_output=cast(str, curve["y_output"]),
        label=cast(str, curve["label"] if "label" in curve else ""),
    )
    ...
    *,
    ...
    if project.linkage_type == RACK_AND_PINION_LINKAGE_TYPE:
        if project.input_mode == "pinion_angle":
            ...
            pitman_left_output_3d = solution.pitman_left_output_3d
            if pitman_left_output_3d is None:
                raise RuntimeError(
                    "Rack-and-pinion solution is missing a left rack output"
                )
            rack_displacement = float(
                pitman_left_output_3d[1] - hardpoints.pitman.left_output[1]
            )
            actuator_outputs = {
...
def project_from_dict(data: dict[str, Any]) -> SteeringProject:
    ...
    return SteeringProject(
        ...
        curves=[_curve_from_data(curve) for curve in data.get("curves", [])],
    )

...
class SuspensionOptimizationTarget:
    ...
    def __post_init__(self) -> None:
        ...
        supported_modes = {
            mode for mode, _label in SUSPENSION_OPTIMIZATION_TARGET_MODES
        }
...
def suspension_pair_delta_constraint_residuals(
    ...
    residuals: list[float] = []
    active_constraints = [
        constraint for constraint in constraints if constraint.enabled
    ]
...
def _variable_parts(variable_name: str) -> tuple[PointID, Axis]:
    ...
    try:
        ...
        raise ValueError(
            f"Unknown hardpoint in optimization variable {variable_name!r}"
        ) from exc
    axis_map = {"x": Axis.X, "y": Axis.Y, "z": Axis.Z}
    try:
        ...
        raise ValueError(
            f"Unknown axis in optimization variable {variable_name!r}"
        ) from exc
...
def _point_id_from_name(point_name: str) -> PointID:
    try:
        ...
        raise ValueError(
            f"Unknown hardpoint in optimization constraint {point_name!r}"
        ) from exc

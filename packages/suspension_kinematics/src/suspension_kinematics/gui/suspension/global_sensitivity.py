...
from suspension_kinematics.gui.common import raise_if_cancelled
...
class MorrisVariableStat:
    """Morris elementary-effect statistics for one optimization variable."""
...
class SobolVariableStat:
    """Sobol first-order and total-order statistics for one variable."""
    ...
    *,
    ...
    for trajectory in range(max(1, trajectories)):
        ...
        for reduced_index in order:
            ...
            effect = (
                float(evaluate_objective(plus)) - float(evaluate_objective(minus))
            ) / (2.0 * step)
            projected = np.abs(
                parameterization.project_direction_to_variables(direction)
            )
            ...
            for variable_index in range(variable_count):
                if weights[variable_index] <= 1e-12:
                    continue
                elementary_effects[variable_index].append(
                    float(effect * weights[variable_index])
                )
    ...
    for index in selected:
        ...
        for _sample in range(max(1, base_samples)):
            ...
            remainder_values.append(
                float(evaluate_objective(parameterization.map_to_variables(point)))
            )
        conditional_variance = float(
            np.var(np.asarray(remainder_values, dtype=np.float64))
        )
...
def pick_reduced_directions_from_morris(
    *,
    ...
    selected = [
        direction_index for score, direction_index in direction_scores if score > 1e-12
    ]
    if not selected:

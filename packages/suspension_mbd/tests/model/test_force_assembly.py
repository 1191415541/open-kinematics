...
def test_force_elements_are_side_paired_and_c_bushings_are_active() -> None:
    ...
    assert sum(isinstance(item, LinearSpringElement) for item in k_assembly.elements) == 2
    assert sum(isinstance(item, VerticalTireElement) for item in k_assembly.elements) == 2
    assert not any(isinstance(item, BushingElement) for item in k_assembly.elements)
    assert any(
        isinstance(item, BushingElement)
        and np.linalg.norm(item.stiffness) > 0

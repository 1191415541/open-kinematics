...
from suspension_mbd.io import META_KEY, read_table, write_bundle
from suspension_mbd.schema import Manifest, Provenance, ResultBundle, StateResult
...
def test_write_and_read_json_parquet_csv(tmp_path: Path) -> None:
    ...
    assert (
        META_KEY.encode()
        in pq.read_schema(tmp_path / "states.parquet").metadata
    )

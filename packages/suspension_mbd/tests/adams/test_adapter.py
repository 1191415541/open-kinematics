...
from suspension_mbd.adams import AdamsBatchAdapter, AdamsProfile, discover_profile
...
def test_full_validation_requires_numeric_baseline(tmp_path: Path) -> None:
    ...
    assert payload["comparison_error"] == "numeric reference is required for the full gate"


def test_full_numeric_comparison_passes(tmp_path: Path) -> None:
    reference = _reference()
    result = AdamsBatchAdapter(_offline_profile(tmp_path), runner=_runner({"groups": reference})).full(
        tmp_path / "pass", reference=reference
    )
    assert result.ok
...
def test_full_numeric_comparison_rejects_missing_group(tmp_path: Path) -> None:
    reference = _reference()
    actual = {"groups": {"K_geometry": reference["K_geometry"], "static_load": reference["static_load"]}}
    result = AdamsBatchAdapter(_offline_profile(tmp_path), runner=_runner(actual)).full(
...
def test_full_numeric_comparison_rejects_reduced_baseline(tmp_path: Path) -> None:
    ...
    result = AdamsBatchAdapter(_offline_profile(tmp_path), runner=_runner({"groups": reference})).full(
        tmp_path / "reduced", reference=reference
    )
    assert not result.ok

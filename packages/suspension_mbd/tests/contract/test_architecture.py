...
import suspension_mbd


def test_public_import_is_independent() -> None:
    assert suspension_mbd.__version__
    assert not any(name == "kinematics" for name in suspension_mbd.__dict__)


def test_business_source_does_not_import_legacy_package() -> None:
    source_root = Path(suspension_mbd.__file__).parent

...
def scale_jacobian(jacobian: np.ndarray, *, translation_scale: float = 1.0, rotation_scale: float = 100.0) -> np.ndarray:
    ...
    if matrix.shape[1] % 6:
        return matrix
    scales = np.tile([translation_scale] * 3 + [rotation_scale] * 3, matrix.shape[1] // 6)
...
def diagnose_rank(jacobian: np.ndarray, *, rtol: float = 1e-10) -> RankDiagnostic:
    ...
    if not scaled.size:
        return RankDiagnostic(0, scaled.shape[1], 0, (), float("inf"), scaled.shape[1] > 0, False)

...
def _template_hardpoints(manifest: Mapping[str, Any]) -> dict[str, tuple[float, float, float]]:
    try:
        raw = manifest["physical_input"]["adams_template_hardpoints_mm"]
    except (KeyError, TypeError) as exc:
        raise ValueError("equivalence manifest lacks Adams template hardpoints") from exc
...
def _replace_hardpoint(
    text: str, name: str, values: tuple[float, float, float]
) -> str:
    number = r"[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[Ee][-+]?\d+)?"
    pattern = re.compile(

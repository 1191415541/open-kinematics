...
def validate_strict_k(
    ...
    *,
    ...
    """Run independent pure-K Adams and suspension_mbd solves and compare all points."""
    destination = (
        Path(evidence_dir)
        if evidence_dir is not None
        else Path(tempfile.mkdtemp(prefix="suspension_mbd_strict_k_evidence_"))
    )
    ...
    try:
        ...
        runtime = Path(tempfile.mkdtemp(prefix="suspension_mbd_strict_k_runtime_"))
        adams_states, execution = run_adams_pure_k(profile, manifest, runtime)
        reference_states = run_suspension_mbd_pure_k(profile)
        ...
        _write_json(
            destination / "suspension_mbd_k_results.json",
...
def build_equivalence_manifest(profile: AdamsProfile) -> dict[str, Any]:
    ...
    physical_input = {
        "coordinates": {
            ...
            "suspension_mbd": "+X rearward,+Y right,+Z up",
            "adams_to_suspension_mbd": [[-1, 0, 0], [0, 1, 0], [0, 0, 1]],
            "length_unit": "mm",
            "angle_unit": "deg",
        },
...
def run_suspension_mbd_pure_k(profile: AdamsProfile) -> list[dict[str, Any]]:
    ...
    for wheel in WHEEL_VALUES_MM:
        for rack in RACK_VALUES_MM:
            ...
            if not solved.equilibrium.converged:
                raise RuntimeError(
                    f"suspension_mbd did not converge for {solved.case_id}"
                )

...
class AdamsBatchAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        if force_absolute_tolerance < 0 or moment_absolute_tolerance < 0:
            raise ValueError("force and moment absolute tolerances must be non-negative")
            ...
            for kind, value in tolerances.items():
                if isinstance(value, Tolerance):
                    ...
                    defaults[kind] = Tolerance(
                        float(values[0]), float(values[1]), defaults.get(kind, Tolerance(0, 0, "")).unit
                    )
    ...
    def smoke(self, output_dir: str | Path | None = None) -> SmokeResult:
        ...
        destination = _destination(output_dir, "suspension_mbd_adams_smoke_")
        ...
        return SmokeResult(True, f"Adams smoke mapping exported to {output}", self.profile, str(output), payload)
    ...
    def full(
        ...
        *,
        ...
        runner is supplied, ``SUSPENSION_MBD_ADAMS_RUNNER`` is used as a command.
        """
        destination = _destination(output_dir, "suspension_mbd_adams_full_")
    ...
    def _run_runner(
        ...
        if configured is None:
            command = os.environ.get("SUSPENSION_MBD_ADAMS_RUNNER")
        ...
        try:
            if callable(configured):
                result = _invoke_callable(configured, self.profile, request_path, destination)
                payload, path = _runner_result(result, destination)
                execution = RunnerExecution(True, True, output_path=str(path) if path else None)
            ...
            (destination / "adams_runner.stdout.txt").write_text(stdout, encoding="utf-8", errors="replace")
            (destination / "adams_runner.stderr.txt").write_text(stderr, encoding="utf-8", errors="replace")
            if completed.returncode != 0:
                error = f"Adams runner exited with code {completed.returncode}"
                return RunnerExecution(True, False, command, completed.returncode, error=error), None, error
            payload, path = _runner_result(None, destination)
            execution = RunnerExecution(True, True, command, completed.returncode, str(path) if path else None)
            return execution, payload if payload is not None else path, None
...
def _destination(output_dir: str | Path | None, prefix: str) -> Path:
    destination = Path(output_dir) if output_dir is not None else Path(tempfile.mkdtemp(prefix=prefix))
...
def _run_command(
    ...
    environment.update(
        {
            "SUSPENSION_MBD_ADAMS_REQUEST": str(request_path),
            "SUSPENSION_MBD_ADAMS_OUTPUT": str(destination),
        }
    )
...
def _runner_result(result: object, destination: Path) -> tuple[object | None, Path | None]:
    ...
    for candidate in (destination / "adams_results.json", destination / "adams_results.csv"):
...
def _read_numeric_groups(source: object) -> dict[str, dict[str, float]]:
    ...
    groups_obj = payload_mapping.get("groups", payload_mapping.get("values", payload_mapping))
    ...
    for group_name, values in groups_obj.items():
        if isinstance(group_name, str) and isinstance(values, (int, float)) and not isinstance(values, bool):
...
def _unit_factor(kind: str, unit: str) -> float | None:
    ...
    if kind == "compliance":
        return {"mm/n": 1.0, "m/n": 1000.0, "deg/n": 1.0, "rad/n": 180.0 / math.pi}.get(normalized)
    return None
...
def _compare_groups(
    ...
    for group in REQUIRED_GROUPS:
        ...
        for field, expected_value in expected.items():
            ...
            relative_error = (
                absolute_error / abs(expected_value) if expected_value != 0 else (0.0 if absolute_error == 0 else math.inf)
            )

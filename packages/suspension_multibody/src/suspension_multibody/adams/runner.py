...
def run_default_adams(
    ...
    command_file = output_dir / "suspension_multibody_adams_validation.cmd"
    ...
    status = output_dir / "suspension_multibody_status.txt"
    if (
        ...
        raise RuntimeError(
            f"Adams batch analysis failed with code {completed.returncode}"
        )

    parallel = (output_dir / "suspension_multibody_validation_parallel.rpt").read_text(
        encoding="utf-8", errors="replace"
    )
    compliance = (output_dir / "suspension_multibody_validation_com.rpt").read_text(
        encoding="utf-8", errors="replace"
    )
    static_result = output_dir / "suspension_multibody_validation_static_load.res"
    ...
    payload = {
        "groups": {
            ...
            "C_compliance": {
                "converging_lateral_steer_symmetry_deg_per_kn": steer_left
                - steer_right,
                "converging_lateral_camber_symmetry_deg_per_kn": camber_left
                - camber_right,
            },
            "static_load": {
...
def _result_component(path: Path, entity_name: str, component_name: str) -> float:
    ...
    if component_id is None:
        raise ValueError(
            f"Adams result channel was not found: {entity_name}.{component_name}"
        )
    data = [
        item
...
acar analysis suspension parallel_travel submit &
 ...
 output_prefix="suspension_multibody_validation" &
...
acar analysis report &
 analysis="suspension_multibody_validation_com" &
...
acar analysis suspension static_load submit &
 ...
 analysis_mode=interactive &
 error_variable=.ACAR.variables.errorFlag
file text open file="suspension_multibody_status.txt" open=overwrite

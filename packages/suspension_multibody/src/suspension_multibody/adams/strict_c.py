...
boundary used by :mod:`suspension_multibody`.  The generated Adams/Car subsystem and
...
def build_strict_c_manifest(profile: AdamsProfile) -> dict[str, Any]:
    ...
    return {
        ...
        "suspension_multibody_snapshot_sha256": snapshot,
...
def validate_strict_c(
    ...
    *,
    ...
    """Run the common compliant model in Adams and suspension_multibody."""
    destination = (
        Path(evidence_dir)
        if evidence_dir is not None
        else Path(tempfile.mkdtemp(prefix="suspension_multibody_strict_c_evidence_"))
    )
    destination.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        runtime = Path(
            tempfile.mkdtemp(prefix="suspension_multibody_strict_c_runtime_")
        )
        adams_states, execution = run_adams_strict_c(profile, model, runtime)
        reference_states = run_suspension_multibody_strict_c(profile)
        ...
        _write_json(
            destination / "suspension_multibody_c_results.json",

...
Moved here from ``analysis/time_domain_physics.py``, which 08 deletes.  This is
a *diagnostic*: it reads the contact loads, poses and wrenches a run already
reports and compares them with finite-difference momentum balance.  It solves
nothing and rebuilds no constitutive law, so it belongs to the report rather
than to the kernel.

The run object stays structurally typed.  The only producer of that shape was
the retired Python vehicle integrator, so the report deliberately does not name
its type: importing the authoring assembly here would be the report ->
preparation edge the boundary forbids.
"""
...
def _center_of_mass(assembly: Any, sample: FullVehicleDynamicSample) -> np.ndarray:
    weighted = np.zeros(3)
    total_mass = 0.0

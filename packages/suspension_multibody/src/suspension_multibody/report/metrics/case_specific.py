...
Moved here from ``metrics/case_specific.py``, which 08 deletes.  The registry,
its single registration entry point and the default family set are unchanged:
``kc_quasi_static``, ``axle_dynamic`` and ``vehicle_dynamic`` have real
implementations, the other five families keep their ``not_applicable``
placeholder.
"""

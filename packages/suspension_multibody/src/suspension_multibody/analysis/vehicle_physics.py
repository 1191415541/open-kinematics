"""
Static vehicle-level load analysis that stays in Python (A2).

``compute_static_wheel_loads`` is the one solver the native kernel does not
replace: there is no static-solve ABI entry (``kernel_abi.cpp`` exposes only
``suspension_kernel_run``) and the export surface is frozen, so the minimum-norm
vertical-load solve stays here and is called by ``vehicle/service.py``.

Everything else that used to live beside it has moved: the wheel-load
aggregation to ``report/wheel_loads.py``, the reporting diagnostics to
``report/``, the K/C signal helpers to ``preparation/signals.py``, the replay
orchestration to ``simulation/replay.py``, and the element force law to the
kernel.  ``compute_vehicle_roll_centers`` has no production caller left and
stays here as a public capability: it calls ``build_vehicle``, so moving it to
``report`` would add a ``report -> preparation`` edge the report boundary
forbids.
"""

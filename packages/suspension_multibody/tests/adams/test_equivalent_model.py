...
from suspension_multibody.adams.equivalent_model import write_equivalent_sources
from suspension_multibody.adams.reference import _read_hardpoints
...
def test_generator_rewrites_manifest_hardpoints_in_an_isolated_suspension_assembly(
    ...
    generated = write_equivalent_sources(
        profile, _manifest(), tmp_path / "runtime", mode="K"
    )

    assert _read_hardpoints(generated.suspension) == _POINTS
    assert "101" in (
        Path(profile.database_path or "") / "subsystems.tbl" / "TR_Front_Suspension.sub"
...
def test_generator_keeps_compliance_mode_in_the_generated_subsystems(
    tmp_path: Path,
) -> None:
    profile = _profile(tmp_path)
    generated = write_equivalent_sources(
        profile, _manifest(), tmp_path / "runtime", mode="C"
    )

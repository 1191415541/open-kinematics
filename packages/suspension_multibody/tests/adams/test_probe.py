...
def test_version_probe_does_not_claim_license_success(
...
def test_discovers_installation_from_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        probe_module, "_run_version_probe", lambda path: ("2024.1", "ok")
    )
    monkeypatch.setattr(
        probe_module, "_run_license_probe", lambda path: ("passed", "ok")
    )
...
def test_license_probe_requires_product_start_marker(
    monkeypatch, tmp_path: Path
) -> None:
    ...
    def run(args, **kwargs):
        ...
        return subprocess.CompletedProcess(
            args=args, returncode=0, stdout="Version = 2024.1_x64", stderr=""
        )
...
def _installation_fixture(root: Path) -> Path:
    ...
    (database / "templates.tbl" / "_double_wishbone.tpl").write_text("fixture")
    (database / "subsystems.tbl" / "TR_Front_Suspension.sub").write_text("fixture")
    (root / "acar" / "acar_report_dictionary.csv").write_text(
        "channel,lcam\nchannel,ltoe\n"
    )
    (root / "bin").mkdir()

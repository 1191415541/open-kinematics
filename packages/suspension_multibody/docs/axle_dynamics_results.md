...
缓存、按约束端点遍历反力、接受试探点后的残差复用和确定性的 worker 缓冲。参考了
Project Chrono `C:/adams_work/chrono-reference` 的隐式求解器组织方式，但没有移植其 NSC
约束或 GMRES 路径；当前密集混合单位 GGL 系统仍使用已验证的 LU 后端。非光滑接触、摩擦、
限位和分段阻尼边界保留原分支语义及必要的差分回退。
...
- **线性代数仍是密集 LU 后端**。当前 134 维 Newton 系统通过跨步缓存复用分解结果；
  正式样本中 53 次分解、4945 次回代，线性求解时间约 `0.188 s`，仍不是必须替换的
  第一瓶颈。Chrono 的矩阵无关 GMRES 路径已做过真实工况试验，但混合单位 GGL 系统的
  收敛稳定性不足，因此未保留。
- **平滑工作区的 Newton Jacobian 已全部解析**。正式样本计数为 `18656` 个解析列、
  `0` 个有限差分列、`0` 个非光滑回退列；运行时方向导数校验和既有物理门仍启用。
  接触、摩擦、限位和分段阻尼切换点没有伪造经典导数，继续使用保持原分支语义的回退。
- **当前主要可见成本**是残量本身（`6945` 次、`0.1276 s`）及其约束 Jacobian/反力
  组装；计时项存在嵌套，不能直接相加为总墙钟。后续若要继续扩大模型规模，优先考虑
...
- 外倾推力、回正力矩、滚动阻力与自旋阻尼为零，除非 manifest 明确给出公式与完整参数。

# 子任务 11：执行独立终局验收并逐条核对 G1-G8

## 目标

独立判定 EPIC `20260922-suspension-template-architecture` 的 Goal 是否达成。

1. **逐条核对 G1–G8**：每条结论指向独立证据（命令退出码或文件），不以子任务自证代替。
2. **执行端到端独立验收**（EPIC Done-When 规定，不依赖子任务完成度）：用**同一个模板**完成四件事——
...
4. **独立列明既有失败**与新增失败：既有失败（skip/xfail/Adams BLOCKED）与 01 基线一致；任何新增失败阻断完成。
...
1. **终局命令全集逐条执行**（EPIC 验证协议的 11 节命令），每条有实测退出码与摘要，缺项为零。
2. **G1–G8 逐条结论**，每条附独立证据（命令退出码或文件路径），且明确区分"达成"/"未达成"/"登记保留"。
3. **端到端四件事 (a)–(d) 全部通过**，每件附命令与输出证据。这是 Goal 达成的终局判据，独立于子任务完成度。
4. **基线重录台账核对**：逐项确认有登记；确认无"改基线让门变绿"的痕迹；确认每次重录都有等价性判定依据。
5. **既有失败独立列明**：与 01 基线对照；新增失败为零；未把未实现的整车 Adams 对标标为通过。
6. **未闭合项显式登记**（若有）：不判为达成，附解除条件。
...
严格按 EPIC Done-When 的 (a)–(d) 执行，每件记录：命令、输出摘要、与预期的对照。
...
1. **以 DONE 代替达成**：所有子任务 DONE 不等于 Goal 达成。端到端四件事必须实际跑。

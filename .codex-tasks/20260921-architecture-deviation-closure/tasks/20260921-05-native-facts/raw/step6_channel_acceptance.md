...
对照方式与数据见 `raw/step7_law_difference_registration.md` 第 1-2 节（同一状态、单位折算、可复现探针）。逐通道判定：
...
| bushing（type 2） | 通道存在，C 模式 32 行/样本 | 承载端力矩差 0.1887–0.5018 N·m（相对 0.108%–0.287%）；固定体端 native 全 NaN 而 Python 给出有限值 | **不通过** |
...
| external / gravity（type 7） | 通道存在，K/C 各 10 行/样本 | K 模式力与力矩恰为 `0.0`（无外力）；两侧均无实体载荷可比 | **不通过**（K 模式无可用元件事实） |

**结论**：五类中无一类通过等价性验收——一类显式不通过（bushing），其余四类因该通道在 `kc_quasi_static` 下无独立可比事实而**不能宣称通过**（不得以「无差异」代替「已验收」）。

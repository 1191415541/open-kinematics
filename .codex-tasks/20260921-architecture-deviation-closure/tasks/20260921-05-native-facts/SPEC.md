# 子任务 05：接管 Python 元件报告事实、静轮荷辅助求解与计算性凝聚

## 目标

让 Python 侧的元件载荷报告、静轮荷与焊接体凝聚都建立在 native 事实上，按三步顺序推进，禁止一次积累全任务 diff：

1. 凝聚与模型映射：04已建立 mb_assembly 的职责边界并交接来源清单；本任务负责把计算性焊接体凝聚迁入 mb_assembly，保持 body ID→凝聚体 ID 映射并验证 native 输出与 Adams 渲染身份。
...
- 新增契约字段只在证明确有输出缺口时采用向后兼容扩展，并记录版本策略；ABI 签名与现有导出不变。
...
2. 缺口一律由 C++ 输出补字段解决；`kernel` 与 `contracts` 测试及版本兼容门通过，无字段被填零。
3. 焊接体凝聚的计算性部分在 `mb_assembly`，body ID→凝聚体 ID 映射有测试，凝聚后实体身份在 native 输出与 Adams 渲染中一致。
4. 静轮荷由 native 静力辅助求解 + `results` 映射提供，保持原最小范数算法与 service 调用语义，并单独有测试。
5. 元件载荷报告经 `results` decoder 从 native 结果读取；通道顺序、单位、方向、参考点与失败证据保持。
6. 冻结的通道级容差验收逐通道通过；动态数组逐位一致、K/C parity、family parity、ABI 七符号门通过。
7. Python 与 native 的力律差异全部登记；旧 Python 路径仍在但已被标记为待 06 删除，未发生第二套力律并存通过验收。

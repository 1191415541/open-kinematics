...
- **凝聚等价性有实测证据**：native `kind="fixed"` 关节与 Python 凝聚的等价性测试 + body ID→凝聚体 ID 映射登记（05 步骤 2，`../20260921-05-native-facts/raw/step2_condensation_equivalence.md`）。
- **数值门为独立项**：T8 动态哈希 26/26 逐位一致（组合哈希 `e7407656731ed556efc28fb89d8fc69881725b3bfb2f39066eb898986389d48e`，与 01 冻结值相同）、T9 K/C parity 0、T10 八 family 0；**未重录任何基线**（`dynamic_hash_baseline.json`、`kc_baseline/`、`vehicle_dynamics_baseline/`、`kc_perf_baseline*.json` 全程未改）。

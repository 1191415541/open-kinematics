...
- **凝聚等价性有实测证据（A3 后更新）**：生产路径已切换为**不凝聚 + weld 送 native `kind="fixed"`**；等价性由实测证据支撑——两条路径总质量均为 `3080.0`、世界质心均为 `[12.987013, 0.0, 160.390]`、`tire_output` 与 `energy` 逐位相同（差异仅表示层：融合体原点移动与新增 `rear_rack` body）。测试 `test_native_fixed_joint_is_what_carries_a_weld`、`test_the_two_weld_routes_agree_on_the_world_mass_properties`；回退开关 `SUSPENSION_MULTIBODY_CONDENSE_WELDS=1` 有测试。登记见 `../20260921-05-native-facts/raw/step2_condensation_equivalence.md`。

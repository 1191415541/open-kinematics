...
3. G2（Python report）：`report` 已建立且旧目录、旧导入、转发壳为零。
4. G3（Python 无求解/本构）：生产代码无关节残差/Jacobian、力元本构或反力求解；元件载荷报告有 native 逐通道证据。
5. G4（保留能力）：公开 API 与 CLI、七个 family、Adams 渲染、历史 artifact 读取与 success/partial/failed artifact 端到端通过。
6. 隔离 wheel 验证：安装后 import/CLI 可用、七个导出符号一致、native 可执行、旧目录缺席。
...
3. G2 证据：`report` 存在且不调用 native、不执行 preparation、不复算本构；删除后扫描显示旧目录、旧导入、转发壳为零。
4. G3 证据：生产代码无关节残差/Jacobian、力元本构或反力求解；元件载荷报告逐通道与 native 输出对照（名称/ID、两端、坐标系、作用点、符号、单位、能量、active 状态）。
5. G4 证据：公开 API 与 CLI、七个 family、Adams source rendering、历史 artifact 读取与 success/partial/failed artifact 端到端通过。
6. 隔离环境安装两个 wheel 后 import/CLI 可用、七个导出符号与 01 冻结清单一致、native 可执行、旧目录缺席。
7. 不相关既有失败独立列明；新增失败为零；未把未实现的整车 Adams 对标标为通过。

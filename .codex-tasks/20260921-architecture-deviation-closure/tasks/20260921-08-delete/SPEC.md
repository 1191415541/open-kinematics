...
1. 删除 `core`、`elements`、`model`、`analysis`、`metrics` 旧目录与旧顶层 `pac2002_scope.py`，不留旧路径转发壳、不重复 native 提交、不留第二统一 decoder。
...
- 删除后必须复跑扫描与门禁；旧目录、旧导入、转发壳残留数为零才算完成。
...
1. `SYMBOL_MATRIX.csv` 每行要么已迁移并指向新归属，要么有明确的删除依据；无未闭合行。
2. 删除前与删除后两次全仓扫描结果留档；扫描器覆盖相对 import、别名 import、动态 import，且排除历史任务记录与生成目录。
3. `core`/`elements`/`model`/`analysis`/`metrics` 旧目录与旧顶层 `pac2002_scope.py` 不存在，无转发壳、无重复 native 提交、无第二统一 decoder。

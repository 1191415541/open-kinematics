...
def _draw_bellcrank(
    ...
    color = PREVIEW_GEOMETRY_COLORS["bellcrank"]
    points = np.vstack([pivot, center_link_pickup, tie_rod_pickup])
    ax.add_patch(
        Polygon(
            points,
            closed=True,
...
def _draw_three_segment_state(
    ax: Axes,
    geometry: ThreeSegmentSteeringGeometry,
    state: ThreeSegmentSteeringSolution,
    alpha: float,
    wheel_radius: float,
    wheel_width: float,
) -> None:
    ...
    _draw_bellcrank(
        ...
        state.left_bellcrank_center_link_pickup,
        state.left_bellcrank_tie_rod_pickup,
        alpha,
    )
    _draw_bellcrank(
        ax,
        geometry.right_bellcrank.pivot,
        state.right_bellcrank_center_link_pickup,
        state.right_bellcrank_tie_rod_pickup,
        alpha,
    )
    _draw_segment(
        ax,
...
def draw_three_segment_steering_preview(
    ax: Axes,
    geometry: ThreeSegmentSteeringGeometry,
    design_state: ThreeSegmentSteeringSolution,
    current_state: ThreeSegmentSteeringSolution,
    *,
    preserve_view: bool = False,
    wheel_radius: float = WHEEL_RADIUS,
    wheel_width: float = WHEEL_WIDTH,
) -> None:
    """Draw the design and current top-view three-segment steering geometry."""
    previous_xlim = ax.get_xlim()
    previous_ylim = ax.get_ylim()
    had_data = ax.has_data()
    ax.clear()
    _draw_three_segment_state(
        ax,
        geometry,
        design_state,
        alpha=0.28,
